...
[ -n "$READARR_API_KEY" ]    && setOpt ApiKey $(lower "$READARR_API_KEY")
[ -n "$READARR_ANALYTICS" ]  && setOpt AnalyticsEnabled $(bool "${READARR_ANALYTICS:-false}")
[ -n "$READARR_LOG_LEVEL" ]  && setOpt LogLevel $(camel "${READARR_LOG_LEVEL:-info}")
[ -n "$READARR_URL_BASE" ]   && setOpt UrlBase "$READARR_URL_BASE"

# Postgres
[ -n "$READARR_POSTGRES_USER" ]    && setOpt PostgresUser "$READARR_POSTGRES_USER"
[ -n "$READARR_POSTGRES_PASS" ]    && setOpt PostgresUser "$READARR_POSTGRES_PASSWORD"
[ -n "$READARR_POSTGRES_PORT" ]    && setOpt PostgresPort "${READARR_POSTGRES_PORT:-5432}"
[ -n "$READARR_POSTGRES_MAIN_DB" ] && setOpt PostgresMainDb "$READARR_POSTGRES_MAIN_DB"
[ -n "$READARR_POSTGRES_LOG_DB" ]  && setOpt PostgresLogDb "$READARR_POSTGRES_LOG_DB"
